...
model {
  ...
  for (s in 1:nSite) {
    # alpha[] passed in as data, e.g. c(0.5,0.5,0.5)
    pi_race[s, 1:nRace] ~ ddirch(alpha[])
  }


  ################################################
  # 2. TRANSPORT MODEL: transport | race, site, time
  #    (second in ordering)
  ################################################
  ...
  # Race effects on transport, last race as reference
  for (r in 1:(nRace-1)) {
    gamma_race[r] ~ dnorm(0, 1.0E-4)
  }
  gamma_race[nRace] <- 0
  ...
  ################################################
  # 3. NOTIFY MODEL: notify | transport, race, site, time
  #    (third in ordering)
  ################################################
  ...
  delta_tr ~ dnorm(0, 1.0E-4)

  # Race effects on notify, last race as reference
  for (r in 1:(nRace-1)) {
    delta_race[r] ~ dnorm(0, 1.0E-4)
  }
  delta_race[nRace] <- 0
  ...
  ################################################
  # 4. AGE MODEL: age | notify, transport, race, site, time
  #    (last in ordering)
  ################################################
  ...
  # Race effects on age, last race as reference
  for (r in 1:(nRace-1)) {
    alpha_race[r] ~ dnorm(0, 1.0E-4)
  }
  alpha_race[nRace] <- 0
...
data_list <- list(
  ...
  home      = as.numeric(x$home_or_rehab),         # 0/1 (if logical, as.numeric)
  age       = x$age,                               # numeric, NA allowed
  notify    = as.numeric(x$pre_hosp_notify),       # 0/1, NA allowed
  transport = as.numeric(x$emsvscar),              # 0/1, NA allowed
  ...
  gender = as.numeric(x$gender) - 1,               # 0/1
  race   = as.numeric(x$race2),                    # 1..nRace, NA allowed

  site   = as.numeric(x$site_id),                  # 1..nSite
  time   = as.numeric(x$time2),                    # 1..nTime
