...
comp1$pred <- predict(fit1, type="response")
...
```{r}
par(mfrow = c(4,5))   # grid of 20 small plots

for (k in 1:20) {
  compk <- complete(imp, k)
  ...
  qqnorm(residuals(fitk, type = "deviance"), 
         main = paste("QQ Deviance Res. - Imp", k), cex = 0.5)
  qqline(residuals(fitk, type = "deviance"))
}
```
