...
model {
  ...
  for (i in 1:N) {
    ...
    # Logistic mixed model with site random intercept
    logit(p[i]) <-
        beta0 +
        beta_age   * age[i] +
        beta_gender * gender[i] +      # assume gender coded 0/1
        ...
        beta_time[ time[i] ] +
        beta_race[race[i]] +           # race coded 1:3
        u_site_out[ site[i] ]
  }
  ...
  # Time effects (quarter) – weakly informative priors
  for (t in 1:nTime) {
    beta_time[t] ~ dnorm(0, 1.0E-4)
  }

  # Race effects with last category as reference
  for (r in 1:(nRace-1)) {
  ...
  sigma_site_out ~ dunif(0, 2)   # small-ish SD prior


  ###############
  # 2. COVARIATE MODEL: AGE
  #    P(age | site, time)  (top of the ordering)
  ###############
  ...
  ###############
  # 3. COVARIATE MODEL: NOTIFY
  #    P(notify | age, site, time)
  ###############
  ...
  ###############
  # 4. COVARIATE MODEL: TRANSPORT (EMS vs car)
  #    P(transport | age, notify, site, time)
  ###############
  ...
  ###############
  # 5. COVARIATE MODEL: RACE
  #    P(race | site)  – Dirichlet–categorical
  ###############
  ...
  for (s in 1:nSite) {
    pi_race[s, 1:nRace] ~ ddirch(alpha[])   # alpha = c(0.5,0.5,0.5) passed in as data
  }
...
data_list <- list(
  ...
  home   = as.numeric(x$home_or_rehab),      # 0/1 (if logical, as.numeric gives 0/1)
  age    = x$age,                            # numeric, with NA
  notify = as.numeric(x$pre_hosp_notify),    # 0/1 with NA
  transport = as.numeric(x$emsvscar),        # 0/1 with NA
  had_tpa   = as.numeric(x$had_tpa),
  had_thromb= as.numeric(x$had_thrombectomy),
  tpa_comp  = as.numeric(x$tpa_complic),
  thr_comp  = as.numeric(x$thr_complic),

  # gender coded 0/1 (you may need to recode from factor)
  gender = as.numeric(x$gender) - 1,

  # race coded 1..nRace, NA where missing
  race   = as.numeric(x$race2),

  # site and time as 1..levels
  site   = as.numeric(x$site_id),
  time   = as.numeric(x$time2),

  # Dirichlet prior parameter for race
  alpha  = rep(0.5, nlevels(x$race2))
)

# Build and run JAGS model
jags_mod <- jags.model(textConnection(model_string),
                       data = data_list,
                       n.chains = 3)

update(jags_mod, 2000)  # burn-in

samp <- coda.samples(jags_mod,
