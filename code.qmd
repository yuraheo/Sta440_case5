...
model {
  ...
  ################################################
  # 5. DERIVED QUANTITIES: baseline (Y1Q1,Y1Q2)
  #    vs final (Y2Q3,Y2Q4)
  ################################################
  # Collect beta_time for baseline quarters
  for (j in 1:nBase) {
    beta_time_base_j[j] <- beta_time[ base_times[j] ]
  }

  # Collect beta_time for final quarters
  for (j in 1:nFinal) {
    beta_time_final_j[j] <- beta_time[ final_times[j] ]
  }

  # Average effects over baseline and final periods
  beta_time_baseline <- mean(beta_time_base_j[1:nBase])
  beta_time_final    <- mean(beta_time_final_j[1:nFinal])
...
```{r}
time_levels <- levels(x$time2)

baseline_times <- which(time_levels %in% c("Y1Q1", "Y1Q2"))
final_times    <- which(time_levels %in% c("Y2Q3", "Y2Q4"))
...
data_list <- list(
  ...
  nBase  = length(baseline_times),
  nFinal = length(final_times),
  base_times  = baseline_times,
