---
title: "STA 440 Case 5: Stroke Outcomes"
...
To begin our analysis, we first sought to determine which variables were most frequently missing. Table 3 reveals that the 4 variables with missing data were, in descending order, patient age, whether the patient was transported to the hospital via car or EMS, if the hospital was notified prior to patient arrival, if they were transported by EMS, and patient race. To determine whether the data is missing completely at random, we investigated whether there were patterns to the missingness. Figures 2 and 3 are heatmaps visualizing the proportion of data missing from the variables. Figure 2 displays the proportion of missingness in each variable by our three time periods, while Figure 3 displays variable missingness by hospital site. Inspection of Figure 2 reveals that data missingness appears worse for certain time quarters, most severely with missing age in the first period. Figure 3 reveals that certain hospitals, namely site IDs 140, 150, and 170, had high variable missingness, with age being the most frequently missing. Because missingness, specifically in age, seems to vary with observed factors like age and time period, it is reasonable to assume that the data is not missing completely at random. The data being missing not at random is unlikely but not impossible based on these graphs. We cannot, for example, conclusively claim that the probability of age being missing does not depend on the unobserved age. However, because there are sites that have age missing completely for all patients, not just selectively older/younger ones, we believe the most likely scenario is the data being missing at random. 
...
\end{aligned}
$$
This model estimates associations between demographic and pre-hospital factors and the probability of a good discharge outcome.
Time-stage effects are coded relative to the baseline period (Y1Q1–Y1Q2), and Caucasian is used as the reference race category.  
...
$$
This lets age vary by site, time, and pre-hospital factors, reflecting real-world care patterns.  
...
$$
A  close to 0 probability is assigned whenTransport[i]=0, representing that notification is effectively unavailable for car arrivals.  
...
$$
Transport patterns may vary by site and program stage.  
...
This allows each site to have its own racial distribution, with a weakly informative Jeffreys prior.  
...
Time effects provided insight into whether outcomes improved over the study period. Compared with baseline, the mid-study period and final periods were associated with roughly 20-30% higher odds of a good discharge outcome. When translated into predicted probabilities for a reference patient (female, Caucasian, median age(70), car arrival, no complications, average site), the model estimates an absolute improvement of about 2-4 percentage points from baseline to the final study period. Taken together with the residual and calibration diagnostics, these findings suggest modest but consistent evidence that outcomes improved over time. 


# Conclusion
This study combined exploratory data analysis with a joint Bayesian modeling framework to identify predictors of good stroke discharge outcomes and to evaluate whether outcomes improved following implementation of a stroke awareness program. Exploratory analyses highlighted systematic missingness in age, transport, and notification variables across time and sites, motivating a joint modeling approach capable of imputing missing covariates coherently within the likelihood.

Posterior inference revealed several strong predictors of patient outcomes. Older age, EMS transport, and treatment complications substantially reduced a patient's likelihood of discharge to home or rehabilitation, while race disparities were also evident. Importantly, the estimated time-stage effects suggest that the program may have contributed to improved patient outcomes: for a reference patient, the predicted probability of a good outcome increased modestly, approximately 2–4 percentage points, from baseline to the final study period.

Overall, the joint Bayesian model provided a principled and flexible framework for handling missing data, characterizing associations, and assessing temporal improvements in outcomes.

# Limitations and Future Directions

Although the joint model imputes missing values probabilistically, it still relies on the assumption that missingness is ignorable given the modeled covariates; if missingness depends on unobserved severity or workflow factors, residual bias may persist. In terms of site-level heterogeneity, substantial relies on the assumption that missingness is ignorable given the modeled covariates.  
Future work could incorporate site-level predictors, patient severity measures, or dynamic time trends to more precisely quantify program effects. Extensions that explore interactions between demographic and clinical variables, or assess whether program benefits were equitably distributed across patient subgroups, would provide additional insight.     
...
```{r tbl-missingvars, echo=FALSE}
#| tbl-cap: "Missingness Per Variable"
#| tbl-pos: H
load("strokeStudy.RData")
...
```{r fig-missingbytime, echo=FALSE}
#| fig-cap: "Missingness by Time Stage"
#| fig-pos: H
...
```{r fig-missbysite, echo=FALSE}
#| fig-cap: "Missingness Aggregated by Site"
#| fig-pos: H
