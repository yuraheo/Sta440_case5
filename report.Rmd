...
# Conclusion
The research combined exploratory analysis with a joint Bayesian modeling framework to understand factors contributing to good stroke care outcomes and to evaluate whether outcomes improved after the implementation of a stroke awarenenss program. EDA revealed systematic missingness patterns across both time and hospital sites, particularly for age and pre-hospital notification, motivating the need for a modeling approach that integrates missing-data handling directly into the likelihood. The joint model achieved this by simultaneously estimating outcome relationships and imputing missing covariates through a hierarchical factorization informed by clinical workflow and site-level structure.  
 
Posterior estimates highlighted several clinically meaningful predictors of patient discharge outcomes. Older age, EMS transport, and post-procedural complications substantially reduced the likelihood of being discharged home or to rehabilitation, while race differencees were also observed, with African American patients experiencing lower odds of favorable outcomes relative to Caucasian patients. Importantly, analysis of time-stage effects provided evidence that outcomes improved following program implementation. For a typical reference patient, predicted probabilities of a good discharge increased modestly, by approximately 2-4 percentage points, from baseline to the final study period.  


# Limitations and Future Directions

Although the joint model imputes missing values coherently, it still relies on the assumption that missingness is ignorable given the modeled covariates. If missingness depends on unobserved factors, bias may remain. In terms of site-level heterogeneity, substantial relies on the assumption that missingness is ignorable given the modeled covariates.  
Future work can expand the model to incorporate dynamic program effects, explore interactions between treatment pathways and demographics, or evaluate whether improvements were distributed equitably across patient subgroups.   
